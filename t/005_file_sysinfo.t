...
if (defined($ENV{'TEST_AUTHOR'})) {
    plan skip_all => 'Author tests enabled.';
	exit;
}
my %testparams;
my $regex = $ARGV[0];

opendir(D, "sysinfos.public/");
my @files = readdir(D);
closedir D;

foreach my $file (@files) {
	if ($file =~ m/\.parameters$/) {
		open(F, "<sysinfos.public/$file");
		while (<F>) {
			my $line = $_;
			chomp($line);
			my @s = split(/;/, $line);
...
foreach my $filename (keys %testparams) {
	...
	subtest "For $filename" => sub {
		...
		ok($bc->get_sysinfo_from_file("sysinfos.public/$filename"),
			"file=$filename, got sysinfo");
		...
		foreach (sort keys %data) {
			...
			if ($k =~ m/int-/) {
				...
				if (!defined($value) && !defined($bc->{'interface'}{$interface}{$configitem})) {
					pass(
						"file=$filename, expected $interface $configitem undefined, got undefined)");
				}
				elsif ($value) {
					ok(
...
"file=$filename, expected $interface $configitem ($value), got ($bc->{'in    terface'}{$interface}{$configitem})"
				...
				ok($length == $value,
					"file=$filename, length($var), expected ($value), got ($length)");
				...
				if (!defined($value) && !defined($bc->{$k})) {
					...
					ok($bc->{$k} eq $value,
						"file=$filename, $k: expected ($value), got ($bc->{$k})");
				}
			} ## end else [ if ($k =~ m/int-/) ]
